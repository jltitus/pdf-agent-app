...
const cardClass =
'rounded-3xl border border-[#d8d1c7] bg-white p-5 shadow-sm sm:p-6'
const statCardClass =
...
const productionReleases = releases.filter((item) => item.status ===
'production')
const qaReleases = releases.filter((item) => item.status === 'qa')
const archivedReleases = releases.filter((item) => item.status ===
'archived')
const plannedReleases = releases.filter((item) => item.status ===
'planned')
...
const passedSmokeTests = smokeTests.filter((item) => item.test_status
=== 'pass')
const failedSmokeTests = smokeTests.filter((item) => item.test_status
=== 'fail')
const blockedSmokeTests = smokeTests.filter((item) => item.test_status
=== 'blocked')
const pendingSmokeTests = smokeTests.filter((item) => item.test_status
=== 'pending')
...
['new', 'open', 'reviewed',
'enhancement_candidate'].includes(item.status)
)
const resolvedIssues = issues.filter((item) => item.status ===
'resolved')
