...
export async function POST(request: Request) {
  try {
    ...
    // Completely remove access request row
    ...
    if (deleteRequestError) {
      return NextResponse.json(
        { error: deleteRequestError.message },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json({
      success: true,
      message:
        'User deleted. Chat history and feedback were kept for reporting.',
    })
  } catch (error: any) {
    return NextResponse.json(
      {
