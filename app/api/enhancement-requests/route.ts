...
function isValidSourceType(value: unknown): value is EnhancementSourceType {
  return (
    value === "manual" || value === "chat_feedback" || value === "issue_report"
  );
}

const enhancementSelect =
  "id, source_type, source_id, title, description, status, priority, is_public, admin_user_id, created_at, updated_at, resolved_at";
...
export async function POST(request: Request) {
  try {
    ...
    if (sourceType !== "manual" && !sourceId) {
      return NextResponse.json(
        { error: "Source id is required for sourced enhancements" },
        { status: 400 },
      );
    }
    ...
    if (error) {
      if (error.code === "23505") {
        return NextResponse.json(
          { error: "An enhancement request already exists for this source." },
          { status: 409 },
        );
      }
      return NextResponse.json({ error: error.message }, { status: 500 });
